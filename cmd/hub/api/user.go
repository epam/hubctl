...
func userDeploymentKey(subject string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error querying HubCTL User Deployment Key: %v", err)
	}
	if code != 200 {
		return "", fmt.Errorf("Got %d HTTP querying HubCTL User Deployment Key, expected 200 HTTP", code)
	}
